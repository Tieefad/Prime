...
function Movies({ darkMode, setDarkMode, onNavigate, user }) {
  ...
  const fetchMovies = async () => {
    setLoading(true);
    try {
      const q = query(collection(db, "events"), where("category", "==", "Movie"), orderBy("createdAt", "desc"));
    ...
    <div style={{ fontFamily: "'Segoe UI', Arial, sans-serif", background: theme.background, color: theme.text, minHeight: "100vh" }}>
      ...
      <div style={{ padding: "30px 40px 20px", textAlign: "center", background: darkMode ? "linear-gradient(135deg, #0a0f1e, #0f172a, #1a1040)" : "linear-gradient(135deg, #e0f2fe, #f0f9ff, #ede9fe)" }}>
        ...
        <div style={{
          ...
          <input placeholder="Search movies..." value={searchQuery}
      ...
      <div style={{ padding: "40px" }}>
        {loading ? (
          ...
          <div style={{ textAlign: "center", padding: "60px", color: theme.subtext }}>
            ...
            <p style={{ fontSize: "18px", fontWeight: "600" }}>No movies found</p>
            <p style={{ fontSize: "14px", marginTop: "8px" }}>Admin needs to add movies with category "Movie"</p>
          </div>
        ) : (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: "24px" }}>
            {filteredMovies.map((e, i) => (
              <div key={e.id} style={getCardStyle(i)} onMouseEnter={() => setHoveredCard(i)} onMouseLeave={() => setHoveredCard(null)}>
                <div style={{ height: "6px", background: "linear-gradient(90deg, #f59e0b, #4facfe)" }} />
                <div style={{ padding: "20px" }}>
                  <div style={{ display: "inline-block", background: "#f59e0b22", color: "#f59e0b", fontSize: "12px", fontWeight: "700", padding: "4px 10px", borderRadius: "999px", marginBottom: "12px" }}>🎬 Movie</div>
